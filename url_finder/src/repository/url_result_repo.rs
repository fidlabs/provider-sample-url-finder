...
impl UrlResultRepository {
    ...
    pub async fn insert_batch(&self, results: &[UrlResult]) -> Result<usize> {
        ...
        let client_ids: Vec<Option<String>> = results
            ...
            .collect();
        let result_types: Vec<String> = results.iter().map(|r| r.result_type.clone()).collect();
        ...
        let result_codes: Vec<String> = results.iter().map(|r| r.result_code.clone()).collect();
        let error_codes: Vec<Option<String>> =
        ...
        let result = sqlx::query!(
            ...
            &result_types as &[String],
            &working_urls as &[Option<String>],
            &retrievability_percents as &[f64],
            &result_codes as &[String],
            &error_codes as &[Option<String>],
